    ...
    public function register(Request $request)
    {
        ...
        //  dd($response);
         ...
         Student::create([
            ...
            'email' =>  $request->email,
            'user_birthday' => $request->date .'/'.$request->month.'/'.$request->years,
            'user_phone' => $request->number11 .'-'.$request->number12.'-'.$request->number13,
            ...
            'user_state' =>  $request->states,
            'user_zipcode' =>  $request->zipcode,
            'source' =>  $request->find,
            'register_time'=>Carbon::now()->format('j F Y'),
            'is_confirmed' =>  1,
         ]);
    ...
    public function register_update( Request $request)
    {
       ...
       $studentData =[
        ...
        if ($request->filled('password_confirmation')) {
          ...
          if(Auth::guard('student')->user()->is_admin == 1){ 
            return redirect()->route('admin.registration')->with('success','student 
            edited successfully');
          }else{
      ...
      if(Auth::guard('student')->user()->is_admin == 1){ 
        return redirect()->route('admin.registration')->with('error', 'Failed to update student. Please try again.');
      }else{
